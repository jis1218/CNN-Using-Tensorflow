...
if __name__ == '__main__':
    ...
    h_conv1 = tf.nn.relu(conv2d(x_image, W_conv1) + b_conv1) #2차원 배열끼리 합성곱을 실행한다. 필터가 32개이므로 출력이 32인 텐서가 나온다.
    h_pool1 = max_pool_2x2(h_conv1) #풀링 계층 실행, 이미지 크기가 [14, 14]가 된다.
    print(shape(h_pool1))
    
    W_conv2 = weight_variable([5, 5, 32, 64]) # 전에서 출력이 32가 나왔으므로 입력이 32가 되고 출력은 64가 된다.
    b_conv2 = bias_variable([64])
    
    h_conv2 = tf.nn.relu(conv2d(h_pool1, W_conv2)+b_conv2)
    print(shape(h_conv2))
    h_pool2 = max_pool_2x2(h_conv2) #두번째 풀링 계층 실행, 이미지 크기가 [7, 7], 출력은 64가 된다.
